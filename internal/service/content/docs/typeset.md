...
To fix this, shadcn created **shadcn/typeset**, and templ-ui ships it. It's one CSS file that styles everything inside a `typeset` container. The file lives in your project, so you can change it directly when you need to.
...
[Build your typeset](/typeset) in the typeset builder. It generates the `typeset.css` file templ-ui ships, with your picks baked into the preset.
...
templ-ui ships the stylesheet as `assets/css/typeset.css`. If you want to tune it visually first, use the [typeset builder](/typeset): pick your fonts and rhythm, preview them on docs, chat, articles, and other real content, then copy the generated `typeset.css` and preset class into your project.
